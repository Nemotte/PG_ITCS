        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                        if (opt_pokemon01s[selectionPointer.flag] not in myPokemon01s) and len(myPokemon01s)<1:
                            myPokemon01s.append(opt_pokemon01s[selectionPointer.flag])

                        elif ((opt_pokemon01s[selectionPointer.flag] not in myPokemon01s) and len(myPokemon01s)>=1):
                            # 玩家选择多于 6 只
                            screen.blit(p_setting.loadingPage, (0, 0))
                            draw_text(screen, p_setting.mainFont, 13, "最多只能选择 1 只 pokemon", (170, 137), (100, 100, 100))

...
class Protection(object):
    ...
    def protectionMove(self):
        if self.pokemon.label == 1:
            ...
            for i in range(100):
                gf.update_screen(self.p_setting, self.screen, self.menubar,
                                 None, self.pokemon, self.target, self.skillImage,
                                 self.menubar)
        else:
            self.skillImage.rect_x = 300
            self.skillImage.rect_y = 35
...
class Splash(object):
    ...
    def use(self):
        damage = self.pokemon.maxHealth - self.pokemon.health
        self.pokemon.health = self.pokemon.maxHealth
        gf.update_screen(self.p_setting, self.screen, self.menubar,
                         "你 造成了 " + str(damage)+" 点伤害", self.target,
                         self.menubar)
...
class Derivation(object):
    def __init__(self, p_setting, screen, menubar):
        ...
        self.target = None
        self.skillImage = SkillImage(pygame.image.load(
            "../othersource/Pic/derivation.png"), 0, 0, p_setting, screen)
        self.proba = 9

    def derivationMove(self):
        if self.pokemon.label == 1:
            self.skillImage.rect_x = 0
            self.skillImage.rect_y = 0
            gf.update_screen(self.p_setting, self.screen, self.menubar,
                             None, self.pokemon, self.target, self.skillImage,
                             self.menubar)
            gf.draw_text(self.target.screen, "../othersource/font/SiYuanMedium.otf",
                         60, str(self.target.func), (300, 40), (200, 100, 100))
...
class Intergral(object):
    def __init__(self, p_setting, screen, menubar):
        ...
        self.target = None
        self.skillImage = SkillImage(pygame.image.load(
            "../othersource/Pic/intergral.png"), 0, 0,
            p_setting, screen)
        self.proba = 9

    def intergralMove(self):
        if self.pokemon.label == 1:
            self.skillImage.rect_x = 0
            self.skillImage.rect_y = 0
            gf.update_screen(self.p_setting, self.screen, self.menubar,
